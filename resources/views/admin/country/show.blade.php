...
@push('scripts')
    ...
    <script type="text/javascript">
        $(function(){
            $(".delete-country").click(function(event){
                        ...
                        <div class="card-body">
                            <table id="example1" class="table table-bordered table-striped">
                                ...
                                <tr>
                                    ...
                                    <th>Country Name</th>
                                    <th>Status</th>
                                    @can('countries.edit_delete', Auth::user())
                                        <th>Actions</th>
                                    @endcan
                                </tr>
                                </thead>
                                ...
                                @foreach($countries as $country)
                                    <tr>
                                        <td>{{ $loop->index + 1 }}</td>
                                        <td>{{ $country->name }}</td>
                                        <td>{{ $country->status ? 'Publish' : 'Unpublish'}}</td>
                                        <td>
                                            <!-- Edit -->
                                            @can('countries.update', Auth::user())
                                            <a class="btn btn-info btn-sm" href="{{ route('country.edit', $country->id) }}"><i class="far fa-edit"></i></a>
                                            @endcan
                                            <!-- End Edit -->

                                            <!-- Delete -->
                                            @can('countries.delete', Auth::user())
                                                <form style="display: inline-block;" id="delete-form-{{ $country->id }}" action="{{ route('country.destroy', $country->id) }}" method="post">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" class="btn btn-danger btn-sm delete-country" id="{{ $country->id }}"><i class="far fa-trash-alt"></i></button>
                                                </form>
                                            @endcan
                                            <!-- End Delete -->
                                        </td>
