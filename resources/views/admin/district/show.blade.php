...
@push('scripts')
    ...
    <script type="text/javascript">
        $(function(){
            $(".delete-district").click(function(event){
                        ...
                        <div class="card-body">
                            <table id="example1" class="table table-bordered table-striped">
                                ...
                                <tr>
                                    ...
                                    <th>Division</th>--}}
                                    <th>Status</th>
                                    {{--@can('districts.edit_delete', Auth::user())--}}
                                        <th>Actions</th>
                                    {{--@endcan--}}
                                </tr>
                                </thead>
                                ...
                                @foreach($districts as $district)
                                    <tr>
                                        ...
                                        <td>{{ $district->status ? 'Publish' : 'Unpublish'}}</td>
                                        <td>
                                            <!-- Edit -->
                                            {{--@can('districts.update', Auth::user())--}}
                                            <a class="btn btn-info btn-sm" href="{{ route('district.edit', $district->id) }}"><i class="far fa-edit"></i></a>
                                            {{--@endcan--}}
                                            <!-- End Edit -->

                                            <!-- Delete -->
                                            {{--@can('districts.delete', Auth::user())--}}
                                                <form style="display: inline-block;" id="delete-form-{{ $district->id }}" action="{{ route('district.destroy', $district->id) }}" method="post">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" class="btn btn-danger btn-sm delete-district" id="{{ $district->id }}"><i class="far fa-trash-alt"></i></button>
                                                </form>
                                            {{--@endcan--}}
                                            <!-- End Delete -->
                                        </td>
